...
import { FaTshirt, FaHome, FaShoppingCart,FaPen } from "react-icons/fa";
...
export default function Header() {
  ...
  return (
    ...
    <Navbar className='border-b-2 shadow-sm bg-[#e6e6e6] sticky top-0 z-10'>
        <Link to='/' className='self-center whitespace-nowrap text-sm sm:text-xl font-semibold '>
            <span className='font-bold text-[#262626] text-xl'>TRENDY GOODS</span>
        </Link>
        ...
        <Navbar.Collapse>
            ...
            <Link to='/products'>
                ...
                <span className={`${path === '/products' ? 'md:text-[#3d52a0]' : ''} flex items-center gap-1 text-[16px]`}><FaTshirt /> Products</span>
                </Navbar.Link>
            </Link>

            <Link to='/about'>
                <Navbar.Link as={'div'} className={`${path === '/products'  ? 'bg-[#3d52a0]' : '' }`} active={path === '/about'}>
                <span className={`${path === '/about' ? 'md:text-[#3d52a0]' : ''} flex items-center gap-1 text-[16px]`}><FaPen /> About Us</span>
                </Navbar.Link>
            </Link>
